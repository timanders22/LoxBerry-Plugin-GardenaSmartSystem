...
function ge($s) { return htmlspecialchars((string) $s, ENT_QUOTES, 'UTF-8'); }
function gt($k) { return gardena_t($k); }
 ...
 * ================================================================== */
$g_muster = '/^tab-(settings|devices|loxone|test|log)$/';
...
function gtoken_erzeugen($cfgfile, &$meldung)
{
    try {
        ...
        $meldung = gt('TOKEN.KEIN_ZUFALL');
        return '';
    }
    if (!gardena_cfg_set($cfgfile, 'TOKEN', $t)) {
        $meldung = gt('TOKEN.SCHREIB_FEHLER') . ' ' . $cfgfile;
        return '';
    }
    return $t;
}
...
if ($gpost && isset($_POST['newtoken'])) {
    $gtokenmsg = gt('TOKEN.NEU_FEHLER');
    $gneu = gtoken_erzeugen($gconfigfile, $gtokenmsg);
    if ($gneu !== '') { $gtokenmsg = gt('TOKEN.NEU_OK'); }
    $g_tab = 'tab-settings';
}
...
if ($gpost && isset($_POST['save'])) {
    ...
    if ($gport < 1 || $gport > 65535) {
        $gfehler[] = gt('EINST.UDPPORT') . ': 1 - 65535';
        $gport = 5005;
    }
    if ($genabled === '1' && ($gcid === '' || $gsec === '')) {
        $gfehler[] = gt('EINST.CLIENT_ID') . ' / ' . gt('EINST.CLIENT_SECRET');
    }
    ...
    if (!$gfehler) {
        ...
        if (gardena_cfg_write($gconfigfile, $gneu)) {
            ...
            $gfehler[] = gt('TOKEN.SCHREIB_FEHLER') . ' ' . $gconfigfile;
        }
        ...
        if ($gsaved && $genabled === '1' && $gcid !== '' && $gsec !== '') {
            ...
            if ($gapi->authenticate()) {
                ...
                if (!empty($glocs)) {
                    ...
                    $gtest = sprintf(gt('TESTMELD.OK'), implode(', ', $gnamen));
                } else {
                    $gtest = sprintf(gt('TESTMELD.KEINE_LOCATION'), $gapi->last_error);
                }
            } else {
                $gtest = sprintf(gt('TESTMELD.FEHLER'), $gapi->last_error);
            }
        }
    }
}
...
<?php if (!$ghatsockets) { ?>
<div class="sm-alert sm-err"><b><?= gt('EINST.SOCKETS_TITEL') ?></b><br><?= gt('EINST.SOCKETS_TEXT') ?></div>
<?php } ?>
<?php if (!$ghatcurl) { ?>
<div class="sm-alert sm-warn"><b><?= gt('EINST.CURL_TITEL') ?></b><br><?= gt('EINST.CURL_TEXT') ?></div>
<?php } ?>
<?php if ($gsaved) { ?><div class="sm-alert sm-ok"><b><?= gt('ALLG.GESPEICHERT') ?></b></div><?php } ?>
<?php if ($gfehler) { ?>
<div class="sm-alert sm-err"><b><?= gt('ALLG.FEHLER') ?></b><br><?= ge(implode(' | ', $gfehler)) ?></div>
<?php } ?>
<?php if ($gtokenmsg !== '') { ?>
<div class="sm-alert <?= stripos($gtokenmsg, 'FEHLER') !== false || stripos($gtokenmsg, 'ERROR') !== false ? 'sm-err' : 'sm-ok' ?>"><?= $gtokenmsg ?></div>
<?php } ?>
<?php if ($gtest !== '') { ?>
<div class="sm-alert <?= strpos($gtest, 'OK') === 0 ? 'sm-ok' : 'sm-err' ?>"><?= ge($gtest) ?></div>
<?php } ?>

<div class="sm-tabs">
    <a class="sm-tab<?= gaktiv('tab-settings') ?>" data-pane="tab-settings" href="index.php?form=settings"><?= gt('REITER.EINSTELLUNGEN') ?></a>
    <a class="sm-tab<?= gaktiv('tab-devices') ?>" data-pane="tab-devices" href="index.php?form=devices"><?= gt('REITER.GERAETE') ?></a>
    <a class="sm-tab<?= gaktiv('tab-loxone') ?>" data-pane="tab-loxone" href="index.php?form=loxone"><?= gt('REITER.LOXONE') ?></a>
    <a class="sm-tab<?= gaktiv('tab-test') ?>" data-pane="tab-test" href="index.php?form=test"><?= gt('REITER.TEST') ?></a>
    <a class="sm-tab<?= gaktiv('tab-log') ?>" data-pane="tab-log" href="index.php?form=log"><?= gt('REITER.LOG') ?></a>
</div>

<!-- ================= Reiter: Einstellungen ================= -->
<div class="sm-pane<?= gaktiv('tab-settings') ?>" id="tab-settings">

<div class="sm-alert sm-info"><b><?= gt('EINST.ZUGANG_TITEL') ?></b> <?= gt('EINST.ZUGANG_TEXT') ?></div>

<form action="index.php" method="post" autocomplete="off">
<input data-role="none" type="hidden" name="save" value="1">
<input data-role="none" type="hidden" name="activetab" value="tab-settings">

<h2><?= gt('EINST.H_ALLGEMEIN') ?></h2>
<div class="sm-row">
    <div>
        <label><?= gt('EINST.PLUGIN_AKTIV') ?></label>
        <select data-role="none" name="enabled">
            <option value="0"<?= $gc['ENABLED'] != '1' ? ' selected' : '' ?>><?= gt('ALLG.NEIN') ?></option>
            <option value="1"<?= $gc['ENABLED'] == '1' ? ' selected' : '' ?>><?= gt('EINST.PLUGIN_AKTIV_JA') ?></option>
        </select>
    </div>
    <div>
        <label><?= gt('EINST.CLIENT_ID') ?></label>
        <input data-role="none" type="text" name="client_id" value="<?= ge($gc['CLIENT_ID']) ?>">
    </div>
    <div>
        <label><?= gt('EINST.CLIENT_SECRET') ?></label>
        <input data-role="none" type="password" name="client_secret" value="<?= ge($gc['CLIENT_SECRET']) ?>">
    </div>
</div>
<div class="sm-small"><?= gt('EINST.SECRET_HINWEIS') ?></div>

<h2><?= gt('EINST.H_UDP') ?></h2>
<div class="sm-row">
    <div>
        <label><?= gt('EINST.UDP_VERSAND') ?></label>
        <select data-role="none" name="udp_enabled">
            <option value="1"<?= $gc['UDP_ENABLED'] != '0' ? ' selected' : '' ?>><?= gt('ALLG.AKTIV') ?></option>
            <option value="0"<?= $gc['UDP_ENABLED'] == '0' ? ' selected' : '' ?>><?= gt('ALLG.AUS') ?></option>
        </select>
    </div>
    <div>
        <label><?= gt('EINST.MINISERVER') ?></label>
        <select data-role="none" name="miniserver">
<?php if (empty($gms)) { ?>
            <option value="1"><?= gt('EINST.KEIN_MINISERVER') ?></option>
<?php } foreach ($gms as $gnr => $gm) { ?>
            <option value="<?= (int) $gnr ?>"<?= (int) $gc['MINISERVER'] === (int) $gnr ? ' selected' : '' ?>><?= ge($gm['Name'] . ' (' . $gm['IPAddress'] . ')') ?></option>
<?php } ?>
        </select>
    </div>
    <div>
        <label><?= gt('EINST.UDPPORT') ?></label>
...
<div class="sm-row">
    <div>
        ...
        <select data-role="none" name="mqtt_enabled">
            <option value="1"<?= $gc['MQTT_ENABLED'] == '1' ? ' selected' : '' ?>><?= gt('EINST.MQTT_EMPFOHLEN') ?></option>
            <option value="0"<?= $gc['MQTT_ENABLED'] != '1' ? ' selected' : '' ?>><?= gt('ALLG.AUS') ?></option>
        </select>
    </div>
    <div>
        <label><?= gt('EINST.MQTT_TOPIC') ?></label>
        <input data-role="none" type="text" name="mqtt_topic" value="<?= ge($gc['MQTT_TOPIC']) ?>" placeholder="gardena">
    </div>
</div>
<div class="sm-small"><?= gt('EINST.MQTT_FORMAT') ?><br><?= gt('EINST.MQTT_ZEICHEN') ?></div>

<button data-role="none" class="sm-btn" type="submit"><?= gt('ALLG.SPEICHERN') ?></button>
</form>

<h2><?= gt('TOKEN.H') ?></h2>
<div class="sm-alert sm-info"><?= gt('TOKEN.TEXT') ?></div>
<label><?= gt('TOKEN.LABEL') ?></label>
<input data-role="none" type="text" value="<?= ge($gc['TOKEN']) ?>" readonly onclick="this.select();">
<div class="sm-legende">
<span><i class="sm-punkt sm-b-aktion"></i> <?= gt('LEGENDE.AKTION') ?></span>
</div>
<div class="sm-knopfreihe">
<form action="index.php" method="post">
    <input data-role="none" type="hidden" name="activetab" value="tab-settings">
    <button data-role="none" class="sm-btn sm-b-aktion" type="submit" name="newtoken" value="1"
            onclick="return confirm(<?= json_encode(html_entity_decode(gt('TOKEN.NEU_FRAGE'), ENT_QUOTES, 'UTF-8')) ?>);"><?= gt('TOKEN.NEU') ?></button>
</form>
</div>
</div>
...
<h2><?= gt('GERAETE.H') ?><?= !empty($gcache['updated']) ? ' (' . gt('GERAETE.LETZTER_ABRUF') . ' ' . ge($gcache['updated']) . ')' : '' ?></h2>
<?php if (empty($gcache['locations']) || !is_array($gcache['locations'])) { ?>
<div class="sm-alert sm-info"><?= gt('GERAETE.KEINE_DATEN') ?></div>
<?php } else { foreach ($gcache['locations'] as $gloc) {
    if (!is_array($gloc) || empty($gloc['devices']) || !is_array($gloc['devices'])) { continue; } ?>
<p><b><?= ge(isset($gloc['name']) ? $gloc['name'] : '?') ?></b></p>
<table class="sm-tbl">
<tr><th><?= gt('GERAETE.SP_GERAET') ?></th><th><?= gt('GERAETE.SP_SERVICES') ?></th><th><?= gt('GERAETE.SP_STATUS') ?></th><th><?= gt('GERAETE.SP_AKKU') ?></th></tr>
...
?>
<tr><td><?= ge(isset($gdev['name']) ? $gdev['name'] : $gdevid) ?></td><td><?= ge($gsvcs) ?></td><td><?= ge($gstate) ?></td><td><?= ge($gbatt) ?></td></tr>
<?php } ?>
</table>
<?php } } ?>
</div>
...
<div class="sm-step"><b><?= gt('LOX.H_EMPFANG') ?></b><br>
<?= sprintf(gt('LOX.EMPFANG'), (int) $gc['UDPPORT'], ge($gc['MQTT_TOPIC'])) ?>
</div>

<div class="sm-step"><b><?= gt('LOX.H_SENDEN') ?></b><br>
<?= gt('LOX.SENDEN') ?>
...
<div class="sm-step"><b><?= gt('LOX.H_BEFEHLE') ?></b>
<table class="sm-tbl">
<tr><th><?= gt('LOX.SP_TYP') ?></th><th><?= gt('LOX.SP_CMD') ?></th><th><?= gt('LOX.SP_BEDEUTUNG') ?></th></tr>
<tr><td rowspan="4"><span class="sm-mono">MOWER_CONTROL</span></td><td><span class="sm-mono">START_SECONDS_TO_OVERRIDE</span></td><td><?= gt('LOX.C_MOWER_START') ?></td></tr>
<tr><td><span class="sm-mono">START_DONT_OVERRIDE</span></td><td><?= gt('LOX.C_MOWER_DONT') ?></td></tr>
<tr><td><span class="sm-mono">PARK_UNTIL_NEXT_TASK</span></td><td><?= gt('LOX.C_MOWER_PARK_NEXT') ?></td></tr>
<tr><td><span class="sm-mono">PARK_UNTIL_FURTHER_NOTICE</span></td><td><?= gt('LOX.C_MOWER_PARK_FURTHER') ?></td></tr>
<tr><td rowspan="3"><span class="sm-mono">VALVE_CONTROL</span></td><td><span class="sm-mono">START_SECONDS_TO_OVERRIDE</span></td><td><?= gt('LOX.C_VALVE_START') ?></td></tr>
<tr><td><span class="sm-mono">STOP_UNTIL_NEXT_TASK</span></td><td><?= gt('LOX.C_VALVE_STOP') ?></td></tr>
<tr><td><span class="sm-mono">PAUSE</span> / <span class="sm-mono">UNPAUSE</span></td><td><?= gt('LOX.C_VALVE_PAUSE') ?></td></tr>
<tr><td><span class="sm-mono">POWER_SOCKET_CONTROL</span></td><td><span class="sm-mono">START_SECONDS_TO_OVERRIDE</span> / <span class="sm-mono">START_OVERRIDE</span></td><td><?= gt('LOX.C_SOCKET_START') ?></td></tr>
</table>
<div class="sm-small"><?= gt('LOX.SEKUNDEN_HINWEIS') ?></div>
</div>

<div class="sm-step"><b><?= gt('LOX.H_SICHER') ?></b><br>
<?= gt('LOX.SICHER') ?>
</div>
</div>
...
<h2><?= gt('REITER.TEST') ?></h2>
<div class="sm-legende">
<span><i class="sm-punkt sm-b-lesen"></i> <?= gt('LEGENDE.LESEN') ?></span>
<span><i class="sm-punkt sm-b-technik"></i> <?= gt('LEGENDE.TECHNIK') ?></span>
<span><i class="sm-punkt sm-b-aktion"></i> <?= gt('LEGENDE.AKTION') ?></span>
</div>

<h3 class="sm-h3"><?= gt('TEST.H_ANSEHEN') ?></h3>
<div class="sm-knopfreihe">
<a class="sm-btn sm-b-lesen" href="/plugins/<?= $gpl ?>/index.php?action=list<?= $gtokenurl ?>" target="_blank"><?= gt('TEST.K_LISTE') ?></a>
<a class="sm-btn sm-b-lesen" href="/plugins/<?= $gpl ?>/index.php" target="_blank"><?= gt('TEST.K_UEBERSICHT') ?></a>
</div>

<h3 class="sm-h3"><?= gt('TEST.H_TECHNIK') ?></h3>
<div class="sm-knopfreihe">
<a class="sm-btn sm-b-technik" href="/admin/system/logmanager.cgi?package=<?= $gpl ?>" target="_blank"><?= gt('TEST.K_LOGS') ?></a>
</div>

<h3 class="sm-h3"><?= gt('TEST.H_AKTION') ?></h3>
<div class="sm-knopfreihe">
<a class="sm-btn sm-b-aktion" href="/plugins/<?= $gpl ?>/index.php?action=refresh<?= $gtokenurl ?>" target="_blank"><?= gt('TEST.K_ABRUF') ?></a>
</div>
<div class="sm-small"><?= gt('TEST.ABRUF_HINWEIS') ?></div>
</div>

<!-- ================= Reiter: Protokoll ================= -->
<div class="sm-pane<?= gaktiv('tab-log') ?>" id="tab-log">
<h2><?= gt('LOG.H') ?></h2>
<div class="sm-small"><?= gt('LOG.TEXT') ?><br><?= gt('LOG.DATEI') ?> <span class="sm-mono"><?= ge($glogfile) ?></span></div>
<?php if ($gloglines) { ?>
<div class="sm-log"><?= ge(implode("\n", $gloglines)) ?></div>
<?php } else { ?>
<div class="sm-alert sm-info"><?= gt('LOG.LEER') ?></div>
<?php } ?>
<div class="sm-legende">
<span><i class="sm-punkt sm-b-aktion"></i> <?= gt('LEGENDE.AKTION') ?></span>
</div>
...
<form action="index.php" method="post">
    ...
    <button data-role="none" class="sm-btn sm-b-aktion" type="submit"><?= gt('LOG.LEEREN') ?></button>
</form>
</div>
</div>
